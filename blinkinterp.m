function [output,nblinks] = blinkinterp(trial,samplerate,th1,th2,bwindow,betblink,type)
if nargin<7
    type='bicubic';
end
        ...
        if t2(j) < intbound || t3(j) > length(trial)-intbound %if blink is too close to trial boundaries, just put nans into it
            ...
            if strcmpi(type,'linear');
                if t1 ~= t2(j) && t3(j) ~= t4 %interp if all points are distinct
                    trial(t2(j):t3(j)) = interp1([t1 t2(j) t3(j) t4],[trial(t1) trial(t2(j)) trial(t3(j)) trial(t4)],t2(j):t3(j));
                elseif t1 == t2(j) && t3(j) ~= t4 %interp if t1 = t2
                    trial(t2(j):t3(j)) = interp1([t2(j) t3(j) t4],[trial(t2(j)) trial(t3(j)) trial(t4)],t2(j):t3(j));
                elseif t1 ~= t2(j) && t3(j) == t4 %interp if t3 = t4
                    trial(t2(j):t3(j)) = interp1([t1 t2(j) t3(j)],[trial(t1) trial(t2(j)) trial(t3(j))],t2(j):t3(j));
                elseif t1 == t2(j) && t3(j) == t4 %interp if t1=t2 and t3=t4
                    trial(t2(j):t3(j)) = interp1([t2(j) t3(j)],[trial(t2(j)) trial(t3(j))],t2(j):t3(j));
                end
            elseif strcmpi(type,'bicubic');
                if t1 ~= t2(j) && t3(j) ~= t4 %interp if all points are distinct
